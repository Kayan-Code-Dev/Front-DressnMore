...
import { Wrench, Search, Plus, Filter, Pencil, Trash2, Eye } from "lucide-react";
...
export function WorkshopPage() {
  ...
  const [rows, setRows] = useState<WorkshopItem[]>([]);
  ...
  const stats = useMemo(() => ({
    ...
    underConstruction: rows.filter((w) => w.status === "under_construction").length,
  }), [rows]);

  const columns = useMemo(
    () => [
      { key: "workshop_code", title: "الكود" },
  ...
  return (
    <div className="w-full space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "إجمالي الورش", value: stats.total },
      ...
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-black">إدارة الورش</CardTitle>
              <CardDescription>عرض وإدارة ورش التفصيل.</CardDescription>
            </div>
          </div>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    rows.map((row) => {
                      ...
                      return (
                        <TableRow key={row.id}>
                          <TableCell className="text-center"><Badge variant="outline" className="font-mono">{row.workshop_code}</Badge></TableCell>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2 justify-center">
                              <Button variant="ghost" size="icon" asChild title="التفاصيل">
                                <Link to={`/workshop/${row.id}`}><Eye className="h-4 w-4" /></Link>
                              </Button>
        ...
        <CardFooter>
          <p className="text-sm text-muted-foreground">إجمالي الورش: <span className="font-bold">{stats.total}</span></p>
        </CardFooter>
      </Card>
      ...
      <Dialog open={dialog !== null} onOpenChange={() => setDialog(null)}>
        <DialogContent className="sm:max-w-md" dir="rtl">
          <DialogHeader>
            <DialogTitle>
              {dialog === "create" ? "إنشاء ورشة" : dialog === "edit" ? `تعديل ${selected?.name ?? "الورشة"}` : "حذف الورشة"}
            </DialogTitle>
