...
export function InvoicesPage() {
  ...
  const columns = useMemo(() => [
    ...
    { key: "issued_on", title: "تاريخ الإصدار" },
  ], []);
        ...
        <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>عرض وإدارة الفواتير في النظام.</CardDescription>
            </div>
          </div>
                  ...
                  {loading ? (<TableSkeletonRows rows={5} cols={columns.length} />) : rows.length > 0 ? (
                    rows.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="text-center font-medium">{row.customer_name}</TableCell>
                        ...
                        <TableCell className="text-center text-muted-foreground">{row.issued_on}</TableCell>
                      </TableRow>
                    ))
                  ) : (
