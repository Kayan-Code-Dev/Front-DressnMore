...
export function TailoringKanbanBoard({ orders }: TailoringKanbanBoardProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 gap-4 w-full max-w-full overflow-x-hidden">
      {KANBAN_STAGES.map((stage) => {
        ...
        return (
          <div key={stage.key} className="min-w-0 w-full">
            <div
            ...
            <div className="space-y-3 min-h-[80px]">
              {columnOrders.length === 0 ? (
                <div className="rounded-xl border border-dashed p-4 text-center text-xs text-muted-foreground" style={{ borderColor: `${stage.color}44` }}>
                  لا توجد أوامر
                </div>
              ) : (
