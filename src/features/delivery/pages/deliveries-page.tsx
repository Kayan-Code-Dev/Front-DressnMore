...
const statusMap: Record<string, { label: string; variant: "success" | "warning" }> = {
  ready: { label: "جاهز", variant: "warning" },
      ...
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-black" style={{ color: "var(--color-text-primary)" }}>إدارة التسليمات</CardTitle>
              <CardDescription>عرض وتتبع عمليات التسليم.</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-3 mb-4">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input value={search} onChange={(e) => handleSearchChange(e.target.value)} placeholder="بحث في التسليمات..." className="pr-9" />
            </div>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (<TableSkeletonRows rows={5} cols={columns.length} />) : rows.length > 0 ? (
                  ...
                  <TableRow><TableCell colSpan={columns.length} className="py-10 text-center text-muted-foreground">لا توجد تسليمات لعرضها.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
        </CardContent>
        <CardFooter><p className="text-sm text-muted-foreground">إجمالي التسليمات: <span className="font-bold">{rows.length}</span></p></CardFooter>
      </Card>
    </div>
  );
}
