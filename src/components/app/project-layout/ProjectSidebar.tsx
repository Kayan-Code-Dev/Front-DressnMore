...
export default function ProjectSidebar({
  ...
  const sidebarPanel = (className: string, style: CSSProperties) => (
    <aside className={className} style={style}>
      {renderNavigation()}
    </aside>
  );

  const desktopSidebar = sidebarPanel(
    "fixed top-0 right-0 bottom-0 z-40 hidden lg:flex flex-col no-print transition-all duration-300",
    {
  ...
  const mobileSidebar = mobileOpen
    ? sidebarPanel(
        "fixed top-0 right-0 bottom-0 z-50 flex lg:hidden flex-col slide-down",
        {
          width: 260,
          background: sidebarBg,
          borderLeft: "1px solid rgba(255,255,255,0.05)",
  ...
  return (
    <>
      {mobileOpen && <div className="sidebar-backdrop lg:hidden" onClick={onMobileClose} />}
      {mobileSidebar}
      {desktopSidebar}
    </>
  );
