...
import { useBranchesShowRoleQueryOptions, useToggleBranchesRolePermissionMutationOptions } from "@/api/permissions-roles/branches/branches.hooks";
...
export function BranchesPermissionsModal({ role, open, onOpenChange }: Props) {
    const { data, isPending } = useQuery(useBranchesShowRoleQueryOptions(role?.id || 0));
    const permissions = data ? data.permissions : [];
    const [togglingId, setTogglingId] = useState<number | null>(null);
    const { mutate: togglePermission } = useMutation(useToggleBranchesRolePermissionMutationOptions());
