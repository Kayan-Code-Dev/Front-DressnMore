...
import {
  useShowRoleQueryOptions,
  useToggleRolePermissionMutationOptions,
...
export function ViewPermissionsModal({ role, open, onOpenChange }: Props) {
  const { data, isPending } = useQuery(useShowRoleQueryOptions(role?.id!));
  const permissions = data ? data.permissions : [];
  const [togglingId, setTogglingId] = useState<number | null>(null);
  const { mutate: togglePermission } = useMutation(useToggleRolePermissionMutationOptions());
